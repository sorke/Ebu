...
$(document).ready(
    function(){
        view("main");
        if ($.inArray("411", roleArray) >= 0) {
			$('#UserAddBt').attr("disabled",false);
		} else {
			$('#UserAddBt').attr("disabled",true);
...
function view(tag){
    ...
    var html='<thead>'+
        ...
        '<th>生日</th>'+
        '<th>邮箱</th>';
        if ($.inArray("412", roleArray) >= 0) {
			html += '<th>编辑</th>';
		}
		if ($.inArray("413", roleArray) >= 0) {
			html += '<th>删除</th>';
		}
		html += '</tr>' +
			'</thead>';
        ...
        success:function(object){
            ...
            for(var i=0;i<len;i++)
            {
                ...
                html+='<tr>'+                   
                    ...
                    '<td>'+email+'</td>';
                    if ($.inArray("412", roleArray) >= 0) {
						html += '<td><a href="UserEdit.html?id='+ObjId+'">编辑</td>';
					}
					if ($.inArray("413", roleArray) >= 0) {
